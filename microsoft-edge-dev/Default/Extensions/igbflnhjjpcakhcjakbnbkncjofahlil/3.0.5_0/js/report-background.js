/* eslint-disable */
(function($) {
  var CFG, exports, _fn, REPORT_TYPE, requestDataArr, defaultData, VIEW_STRATEGY, SERVER_URL, utmData, VERSION, UserInfo, CUSTOM_HREF, CID, CUSTOM_HOST, EXTENSION_ID, pluginSource, CHANNEL_ID;
  requestDataArr = [];
  CFG = {
    DELAY: 500,
    API_VERSION: '0.6.0',
  ...
  CHANNEL_ID = localStorage.getItem('channelId') || '' 
    ...
    sendAjaxRequest: function(request) {
      chrome.storage.local.get('installUUID', function(items) {
        ...
        if (data.data.hasOwnProperty('__logs__')) {
          // console.log('Report Bg上报', data.data['__logs__'][0].cid || items.installUUID); 
          data.data['__logs__'] = data.data['__logs__'].map(v => ({
            ...v,
            cid: v.cid || items.installUUID,
            did: v.cid || items.installUUID 
          }));
        } else {
          // console.log('Report Bg上报', request.data.cid || items.installUUID);  
          data = Object.assign(data, {
            data: {
              ...request.data,
              cid: request.data.cid || items.installUUID,
              did: request.data.cid || items.installUUID,
            },
          });
        }
    ...
    getUserInfo: function() {
      return new Promise(resolve => {
        ...
        chrome.cookies.getAll({ url: 'https://csdn.net' }, function(res) {
          const userInfo = res
            .filter(v => [ 'UserNick', 'UserName', 'uuid_tt_dd', '_javaeye_cookie_id_'].indexOf(v.name) > -1)
            .reduce((pre, cur) => {
              return pre ? { ...pre, [cur.name]: cur.value } : { [cur.name]: cur.value };
            }, '');
          if (userInfo) {
            UserInfo = userInfo;
            CID = userInfo.uuid_tt_dd || userInfo._javaeye_cookie_id_ || '' 
          } else {
            UserInfo = { UserNick: '', UserName: '' };
          }
