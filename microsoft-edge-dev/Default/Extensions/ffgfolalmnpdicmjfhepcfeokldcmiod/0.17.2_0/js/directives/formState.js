		...
		return {
			...
			link: function(scope, elem, attrs) {
				...
				input.on('focus', function() {
					if(field() && field().$stateVisible) field().$stateVisible = false;
					...
					if (field() && field().$dirty) {
						if(field().$stateVisible) field().$stateVisible = true;
						return safeApply.apply(scope)
					}
				});
				hasError = function(scope) {
					if(scope[form][fieldName] && scope[form][fieldName].$stateVisible){
						return scope[form][fieldName].$invalid;
					}
					return false;
